...
class Amqp extends Component {
     ...
     *
     * @param string $exchange channel名称
     ...
     * @return void
     */
    public function send($exchange, $queue, $routing_key, $message, $type = self::TYPE_TOPIC) {
    ...
    public function testDirect($exchange, $queue, $message, $type = self::TYPE_TOPIC) {
        $message = $this->prepareMessage($message);
        $this->channel->exchange_declare($exchange, $type, false, true, false);
        $this->channel->queue_declare($queue, false, false, false, false);
        $this->channel->queue_bind($queue, $exchange);
    ...
    public function testOne($exchange, $queue, $routing_key, $message, $type = self::TYPE_TOPIC) {
        $message = $this->prepareMessage($message);
        $this->channel->exchange_declare($exchange, $type, false, true, false);
        $this->channel->queue_declare($queue, false, false, false, false);
        $this->channel->queue_bind($queue, $exchange);
    ...
    public function testGet($exchange, $queue, $type = self::TYPE_TOPIC) {
        $this->channel->exchange_declare($exchange, $type, false, true, false);
